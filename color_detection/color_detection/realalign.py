...
class RealSensePublisher(Node):
    ...
    def publish_frames(self):
        ...
        # 프레임 출력 (디버깅용)
        self.show_aligned_images(depth_image, color_image)

        # 1ms 대기 (디버깅용으로 화면 갱신)
        cv2.waitKey(1)

    def show_aligned_images(self, depth_image, color_image):
        # 깊이 이미지와 컬러 이미지를 겹쳐서 보여줌
        depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(depth_image, alpha=0.03), cv2.COLORMAP_JET)
        combined_image = cv2.addWeighted(depth_colormap, 0.7, color_image, 0.3, 0)
        cv2.imshow("Aligned Depth and Color Image", combined_image)
