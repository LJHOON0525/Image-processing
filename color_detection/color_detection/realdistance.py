...
class RealSenseDepthMask(Node):
    def __init__(self):
        ...
        self.mask_pub = self.create_publisher(Image, '/mid_mask', 10)  # 중간 거리 마스크 토픽
        ...
        # 거리 설정
        self.near_distance = 1000  # 1m (단위: mm)
    ...
    def process_frames(self):
        ...
        # 중간 거리 마스크 생성 (1m ~ 2m 사이의 객체만 남김)
        mid_mask = np.where((depth_image >= self.near_distance) & (depth_image <= self.far_distance), 255, 0).astype(np.uint8)

        # 컬러 이미지에 마스크 적용
        masked_color = cv2.bitwise_and(color_image, color_image, mask=mid_mask)

        # ROS2 퍼블리시
        self.mask_pub.publish(self.bridge.cv2_to_imgmsg(mid_mask, encoding="mono8"))
        self.masked_color_pub.publish(self.bridge.cv2_to_imgmsg(masked_color, encoding="bgr8"))

        # 디버깅용 출력
        cv2.imshow("Mid Mask (1m ~ 2m)", mid_mask)
