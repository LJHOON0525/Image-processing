...
class D435iRtabmapRvizNode(Node):
    def __init__(self):
        ...
        odometry_cmd = [
            ...
            "--remap", "imu:=/imu/data",###손봐야됨...굳이 안봐도됨
        ...
        imu_cmd = [
            ...
            "-p", "world_frame:=map",
            "-p", "publish_tf:=false",
            "--remap", "imu/data_raw:=/camera/camera/imu" #이것도 애매
        ]
        ...
        rtabmap_cmd = [
            ...
            "-p", "subscribe_depth:=true",
            "-p", "subscribe_odom_info:=true",
            "-p", "approx_sync:=false", #true로 해도됨, 언제: 로봇에 달때.
            "-p", "publish_tf:=true",
            "-p", "qos:=1",
            "-p", "use_action_for_goal:=true",
            #"-p", "DatabasePath:=/home/ljh/rtabmap_ws/db/my_slam.db",
            "--remap", "rgb/image:=/camera/camera/color/image_raw",
            "--remap", "rgb/camera_info:=/camera/camera/color/camera_info",
            "--remap", "depth/image:=/camera/camera/aligned_depth_to_color/image_raw",
            "--remap", "odom:=/odom/camera"
