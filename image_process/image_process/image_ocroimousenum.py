    ...
    def image_callback(self, msg):
        ...
        if self.roi:
            ...
            if w > 0 and h > 0:
                ...
                # 📌 OCR 실행 (숫자만 인식하도록 설정)
                custom_config = r'--oem 3 --psm 6 -c tessedit_char_whitelist=0123456789'  # 숫자만 인식
                text = pytesseract.image_to_string(thresh, config=custom_config, lang="eng")

                # 📌 결과 출력
                self.get_logger().info(f"OCR Result (Numbers Only): {text}")
