...
class YellowRightLaneWebcam(Node):
    ...
    def yellow_detection(self, img):
        ...
        for contour in sorted(contours, key=cv2.contourArea, reverse=True):
            ...
            if center > w // 2 and right_center is None:
                # 오른쪽 차선
                right_center = center
            ...
            elif center <= w // 2 and left_center is None:
                # 왼쪽 차선 시각화만
                left_center = center
                cv2.drawContours(img, [contour], -1, (255, 255, 0), 2)
                cv2.line(img, (x, 0), (x, h), (0, 255, 255), 1)
                cv2.line(img, (x + w_rect, 0), (x + w_rect, h), (0, 255, 255), 1)
                cv2.line(img, (center, int(h*0.7)), (center, int(h*0.9)), (0, 255, 0), 2)

        if right_center is not None:
            offset = int(w * 0)  # 필요시 offset 조절 가능
            self.center_x = max(right_center - offset, 0)
        else:
            self.center_x = None
            R_sum = 0

        return self.center_x, R_sum, left_center, right_center
    ...
    def image_processing(self):
        ...
        center, self.R_sum, left_c, right_c = self.yellow_detection(roi)
