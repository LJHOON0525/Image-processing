...
class BlueRatioCirculator(Node):
    ...
    def image_processing(self):
        ...
        if results and results[0].boxes.xywh.numel() > 0:
            ...
            if (self.finish_ROI[0][0] < center_x < self.finish_ROI[1][0] and self.finish_ROI[0][1] < center_y < self.finish_ROI[1][1]):
    ...
    def track_tracking(self):
        ...
        if self.chess_detection_flag:
            ...
            roll_offset = self.theta - 90.0
            correction = 0.0

            if abs(roll_offset) >= self.roll_angle_tolerance:
                correction = roll_offset * self.Kp_roll
                correction = np.clip(correction, -self.max_speed * 0.4, self.max_speed * 0.4)

            if detect_sum < 100:
                base_speed = self.max_speed * 0.3
                self.L_joy = base_speed + correction
                self.R_joy = base_speed - correction
                self.get_logger().info(f"전방에 파란색 거의 없음 → 천천히 직진 + 보정 (theta={self.theta:.2f})")
            elif abs(self.L_sum - self.R_sum) < detect_sum * 0.1:
                base_speed = self.max_speed * 0.5
                self.L_joy = base_speed + correction
                self.R_joy = base_speed - correction
                self.get_logger().info(f"직진 보정 적용: theta={self.theta:.2f}, correction={correction:.2f}")
