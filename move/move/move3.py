...
class BlueRatioCirculator(Node):
    def __init__(self):
        ...
        self.U_detection_threshold = 135  # 140 white balance
        self.img_size_x = 848
        self.img_size_y = 480
        ...
        self.center_x = int(self.img_size_x / 2)  # 중심
        self.L_sum = 0
        self.R_sum = 0
        ...
        color_sensor.set_option(rs.option.enable_auto_white_balance, 0)  # 자동 OFF
        color_sensor.set_option(rs.option.white_balance, 6500)  # 수동 화이트밸런스
    ...
    def yuv_detection(self, img):
        ...
        # Y, U, V 범위 조건 설정 (혼합된 초록색 계열 예시)
        y_lower, y_upper = 0, 0
        u_lower, u_upper = 120, 140
        v_lower, v_upper = 0, 0

        # 마스크 생성
        y_mask = cv2.inRange(Y_img, y_lower, y_upper)
        u_mask = cv2.inRange(U_img, u_lower, u_upper)
        v_mask = cv2.inRange(V_img, v_lower, v_upper)

        # YUV 조건을 모두 만족하는 마스크
        combined_mask = cv2.bitwise_and(y_mask, cv2.bitwise_and(u_mask, v_mask))
        ...
        if max_contour is not None:
            max_contour_mask = np.zeros_like(combined_mask)
            ...
            cv2.line(self.color_img, (self.center_x, int(self.img_size_y * 0.7)),
                     (self.center_x, int(self.img_size_y * 0.9)), (255, 0, 0), 2)
