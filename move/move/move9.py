...
class BlueRatioCirculator(Node):
    ...
    def dehaze(self, img):
        ...
        numpx = max(1, int(img.size * 0.001))
        ...
        transmission = np.clip(1 - omega * (dark_channel / 255.0), 0.4, 1.0)

        J = np.empty_like(img_float)
        for c in range(3):
            J[:,:,c] = (img_float[:,:,c] - A[c]) / transmission + A[c]
    ...
    def image_capture(self):
        ...
        # 안개 제거 적용
        self.color_img = self.dehaze(self.color_img)
    ...
    def image_processing(self):
        ...
        for box in goodbox_results[0].boxes.xyxy:
            ...
            cv2.rectangle(self.color_img, (x1, y1), (x2, y2), (130, 135, 0), 2)
