...
class MultiImagePublisherAutumn(Node):
    def __init__(self):
        super().__init__('multi_image_publisher_autumn')
        ...
        self.sub_side_camera = self.create_subscription(Image, '/side_camera', self.side_camera_callback, qos) #가을
        ...
        self.pub_side_camera = self.create_publisher(CompressedImage, '/side_camera/compressed', qos)
    ...
    def side_camera_callback(self, msg):
        self._publish_compressed(msg, self.pub_side_camera, "/side_camera/compressed")
        


def main(args=None):
    rclpy.init(args=args)
    node = MultiImagePublisherAutumn()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
