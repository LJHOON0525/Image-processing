...
class MultiImagePublisherSpring(Node):
    def __init__(self):
        super().__init__('multi_image_publisher_spring')
        ...
        self.sub_yolo_detection = self.create_subscription(Image, '/yolo_detection_image', self.yolo_detection_callback, qos) #봄
...
def main(args=None):
    ...
    node = MultiImagePublisherSpring()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
